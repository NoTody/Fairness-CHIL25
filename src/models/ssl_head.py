...
class SSLHead(nn.Module):
    def __init__(self, config, upsample="vae", swinunetr=True, dim=768):
        super(SSLHead, self).__init__()
        in_channels = 1
        patch_size = ensure_tuple_rep(2, config.MODEL.SPATIAL_DIMS)
        window_size = ensure_tuple_rep(7, config.MODEL.SPATIAL_DIMS)
