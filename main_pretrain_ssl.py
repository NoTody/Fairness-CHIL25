...
from src.losses.ssl_loss import SSLLoss

import numpy as np

from monai.config import print_config

from engine_pretrain_ssl import *
...
def main(config, wandb_run):
    ...
    # Get transforms
    imtrans = ssl_transforms(config, mode='train')
    imvals = ssl_transforms(config, mode='val')
    imtests = ssl_transforms(config, mode='test')
    ...
    # Create loss
    loss = SSLLoss(config, batch_size * num_samples)
    ...
    logger.info(f"train completed, best total loss: {val_loss_min:.4f} ")
