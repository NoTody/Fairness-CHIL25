...
def main(config, wandb_run):
    ...
    # Create data loaders
    _, _, test_loader = get_dataloaders(config, augs=[imtrans, imvals, imtests], filter=True)

    # Create model
    model = get_model(config).to(device)

    # Load model with wrong size weights unloaded
    _, _, model = load_model(config, model, logger, mode='test')
    ...
    logger.info(f"test completed, test average dice: {test_acc_max:.4f} ")
